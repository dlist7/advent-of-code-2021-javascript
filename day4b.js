...
async function main() {
    ...
    // Remove each card with a bingo until there's only one left.
    while (cards.length > 1) {
        let n = numbers.shift();
        callNumber(n, cards);
        cards = cards.filter(c => !hasBingo(c));
    }

    // Once only one is left the solution is the same as the first part.
    let n;
    for (n of numbers) {
        callNumber(n, cards);
